        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if 0 <= neighbor[0] < rows and 0 <= neighbor[1] < cols and clearance_grid[neighbor]:
                ...
                if neighbor not in cost_so_far or new_cost < cost_so_far[neighbor]:
                    ...
                    priority = new_cost + distance.euclidean(neighbor, end)
...
# Generate a smooth connection between paths
def smooth_connection(path1, path2):
    if not path1 or not path2:
        return path1 + path2

    connection_start = path1[-1]
    connection_end = path2[0]

    # Interpolate a smooth connection
    x = [connection_start[0], connection_end[0]]
    y = [connection_start[1], connection_end[1]]
    tck, _ = splprep([x, y], s=2)
    u_fine = np.linspace(0, 1, 10)  # 10 points for smooth transition
    x_smooth, y_smooth = splev(u_fine, tck)
    return path1 + list(zip(x_smooth, y_smooth)) + path2[1:]
...
if not os.path.exists(grid_file_path):
    ...
    try:
        ...
        # Smooth the connection between adjusted segments
        final_path = smooth_connection([], adjusted_path)
        ...
        ax_map.plot(final_path_np[:, 0], final_path_np[:, 1], 'r-', linewidth=2, label="Final Adjusted Path")
        ax_map.legend()
        plt.show()
