...
def smooth_path(path):
    if len(path) < 3:  # Skip smoothing for very short paths
        return path
    ...
    tck, _ = splprep([x, y], s=2)  # Adjust smoothing factor (s)
    u_fine = np.linspace(0, 1, len(x) * 10)  # More points for smoother curve
